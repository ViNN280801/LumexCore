...
{
  {
    ...
    if(level >= LumexLogLevel::Debug && level <= LumexLogLevel::Warning)
      {
        std::clog << _levelToColor(level) << "["
                  << LumexTime::get_current_datetime() << "] "
                  << "|" << std::setw(KLOG_WIDTH) << _levelToString(level)
                  << "| " << moduleName << " : " << msg << "\033[0m"
                  << "\n";
      }
    else
      {
        std::cerr << _levelToColor(level) << "["
                  << LumexTime::get_current_datetime() << "] "
                  << "|" << std::setw(KLOG_WIDTH) << _levelToString(level)
                  << "| " << moduleName << " : " << msg << "\033[0m"
                  << "\n";
...
LumexLogging::getLogsDirectory()
{
  try
    {
#if LUMEX_OS_UNIX
      Check if we're running in AppImage mode std::string appImageMode
        = LumexEnvironment::get("LUMEX_APPIMAGE_MODE").value;
      std::string externalLogsDir
        = LumexEnvironment::get("LUMEX_EXTERNAL_LOGS_DIR").value;

      if(!appImageMode.empty() && appImageMode == "1"
         && !externalLogsDir.empty())
        {
      ...
      if(homeDir.empty())
        {
          ...
          return Lumex::Filesystem::current_path().value_or(
            Lumex::Filesystem::temp_directory_path()
              .value()); // Fallback, but likely to fail permissions
        }

      Lumex::Path userLogsDir
        = Lumex::Path(homeDir) / Lumex::Path(".local") / Lumex::Path("share")
