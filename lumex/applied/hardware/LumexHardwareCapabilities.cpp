...
HardwareCapabilities::detectHardware()
{
  ...
  if(GlobalMemoryStatusEx(&memInfo) != 0)
    {
      info.totalMemoryMB = static_cast<uint64_t>(
          ...
          if(SUCCEEDED(pAdapter->GetDesc(&desc)))
            {
              ...
              if(gpuName.find(Constants::KMICROSOFT_GPU_IDENTIFIER)
                   == std::string::npos
                 && gpuName.find(Constants::KBASIC_GPU_IDENTIFIER)
                      == std::string::npos
  ...
  LumexLogging::info(
    KMODULE_NAME, "Detected hardware - CPU: ", info.cpuName, " (",
    std::to_string(info.cpuCoreCount), " cores, ",
    std::to_string(info.cpuFrequencyMHz), " MHz), ",
    "RAM: ", std::to_string(info.totalMemoryMB), " MB, ",
    "GPU: ", info.hasDiscreteGPU ? info.gpuName : "Integrated/None",
    ", CPU Generation: ~", std::to_string(info.cpuGeneration));
...
HardwareCapabilities::estimateCPUGeneration(std::string const &cpuName)
{
  ...
  if(cpuName.find(Constants::KINTEL_CORE2_IDENTIFIER) != std::string::npos)
  ...
  if(cpuName.find(Constants::KAMD_RYZEN_IDENTIFIER) != std::string::npos)
    {
      if(cpuName.find(Constants::KAMD_RYZEN_1000_IDENTIFIER)
         != std::string::npos)
        return Constants::KCPU_RYZEN_1000_GENERATION;
      if(cpuName.find(Constants::KAMD_RYZEN_2000_IDENTIFIER)
         != std::string::npos)
        return Constants::KCPU_RYZEN_2000_GENERATION;
      if(cpuName.find(Constants::KAMD_RYZEN_3000_IDENTIFIER)
         != std::string::npos)
        return Constants::KCPU_RYZEN_3000_GENERATION;
      if(cpuName.find(Constants::KAMD_RYZEN_5000_IDENTIFIER)
         != std::string::npos)
        return Constants::KCPU_RYZEN_5000_GENERATION;
      if(cpuName.find(Constants::KAMD_RYZEN_7000_IDENTIFIER)
         != std::string::npos)
...
HardwareCapabilities::isOldCPU(std::string const &cpuName)
{
  ...
  std::vector<std::string> const oldCPUs
    = {Constants::KOLD_CPU_PENTIUM,        Constants::KOLD_CPU_CELERON,
       Constants::KINTEL_CORE2_IDENTIFIER, Constants::KOLD_CPU_ATOM,
       Constants::KOLD_CPU_ATHLON64,       Constants::KOLD_CPU_PHENOM,
...
HardwareCapabilities::shouldUseSoftwareRendering()
{
  ...
  if(info.totalMemoryMB < Constants::KMIN_MEMORY_MB)
    {
      useSoftware = true;
      reason += "Low RAM (" + std::to_string(info.totalMemoryMB) + " MB < "
  ...
  if(info.cpuCoreCount < Constants::KMIN_CPU_CORES)
    {
      useSoftware = true;
      reason += "Few CPU cores (" + std::to_string(info.cpuCoreCount) + " < "
  ...
  if(!info.hasDiscreteGPU)
    {
      ...
      if(info.totalMemoryMB < Constants::KHIGH_MEMORY_THRESHOLD_MB
         || info.cpuCoreCount < Constants::KHIGH_CPU_CORES)
        {
          useSoftware = true;
...
HardwareCapabilities::applyOptimalRenderingSettings()
{
  if(shouldUseSoftwareRendering())
    ...
    {
      LumexLogging::info(
        KMODULE_NAME, "Hardware rendering will be used (default Qt settings)");
    }
}
