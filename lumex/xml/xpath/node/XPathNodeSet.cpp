                       ...
                       type_t type_)
{
  ...
  auto size_ = static_cast<std::size_t> (end_ - begin_);

  // use internal buffer for 0 or 1 elements, heap buffer otherwise
  XPathNode *storage
      = (size_ <= 1) ? m_storage.data ()
...
XPathNodeSet::_move (XPathNodeSet &rhs) LUMEX_NOEXCEPT
{
  ...
  m_begin = (rhs.m_begin == rhs.m_storage.data ()) ? m_storage.data ()
                                                   : rhs.m_begin;
...
{
  auto capacity = static_cast<std::size_t> (m_eos - m_begin);
  ...
  auto *data = static_cast<XPathNode *> (
      alloc->reallocate (m_begin, capacity * sizeof (XPathNode),
                         new_capacity * sizeof (XPathNode)));
  if (data == nullptr)
    return;
                         ...
                         XPathAllocator *alloc)
{
  ...
  auto size_ = static_cast<std::size_t> (m_end - m_begin);
  auto capacity = static_cast<std::size_t> (m_eos - m_begin);
  auto count = static_cast<std::size_t> (end_ - begin_);
  ...
  if (m_type == XPathNodeSet::type_unsorted && m_end - m_begin > 2)
    {
      ...
      void const **hash_data = static_cast<void const **> (
          alloc->allocate (hash_size * sizeof (void *)));
      if (hash_data == nullptr)
        return;

      memset (hash_data,
              0, // NOLINT(bugprone-multi-level-implicit-pointer-conversion)
      ...
      for (XPathNode *it = m_begin; it != m_end; ++it)
        {
          ...
          if (key != nullptr
              && xml::utility::hash_insert (hash_data, hash_size, key))
            *write++ = *it;
        }
    ...
    {
      m_end = std::unique (m_begin, m_end);
    }
}
