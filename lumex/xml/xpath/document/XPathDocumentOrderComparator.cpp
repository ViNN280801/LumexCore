    ...
    XmlNodeBase *rn_node) // NOLINT(misc-use-internal-linkage)
{
  ...
  // different documents
  if (ln_node->parent == nullptr)
    return ln_node < rn_node;
    ...
    XmlNodeBase *rn_node) // NOLINT(misc-use-internal-linkage)
{
  ...
  while (lp_node != nullptr && rp_node != nullptr
         && lp_node->parent != rp_node->parent)
    {
  ...
  if (lp_node != nullptr && rp_node != nullptr)
    return xpath::document::node_is_before_sibling (lp_node, rp_node);
    ...
    XPathNode const &xnode) // NOLINT(misc-use-internal-linkage)
{
  ...
  if (node != nullptr)
    {
      if ((xml::document::get_document (node).header
  ...
  if (attr != nullptr)
    {
      if ((xml::document::get_document (attr).header
           & kxml_memory_page_contents_shared_mask)
          == 0)
        {
          if ((attr->header & kxml_memory_page_name_allocated_or_shared_mask)
              == 0)
            return attr->name;
          if ((attr->header & kxml_memory_page_value_allocated_or_shared_mask)
              == 0)
            return attr->value;
        }
    ...
    XPathNode const &lhs, XPathNode const &rhs) const
{
  ...
  if (lo_doc != nullptr && ro_doc != nullptr)
    return lo_doc < ro_doc;
  ...
  if (lhs.attribute () != nullptr && rhs.attribute () != nullptr)
    {
      ...
      if (lhs.parent () == rhs.parent ())
        {
          // determine sibling order
          for (lumex::xml::attribute::XmlAttribute attr = lhs.attribute ();
               attr != nullptr; attr = attr.next_attribute ())
            if (attr == rhs.attribute ())
              return true;
