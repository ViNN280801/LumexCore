...
XPathVariableSet::_assign (XPathVariableSet const &rhs)
{
  ...
  for (std::size_t i = 0; i < sizeof (_data) / sizeof (_data.at (0));
       ++i) // NOLINT(bugprone-sizeof-expression)
    if ((rhs._data.at (i) != nullptr)
        && !_clone (rhs._data.at (i), &temp._data.at (i)))
      return;
...
XPathVariableSet::_find (char_t const *name) const
{
  std::size_t const hash_size
      = sizeof (_data)
        / sizeof (_data.at (0)); // NOLINT(bugprone-sizeof-expression)
  std::size_t hash = utility::hash_string (name) % hash_size;

  // look for existing variable
  for (XPathVariable *var = _data.at (hash); var != nullptr;
       var = var->next ())
    {
      char_t const *tmp = var->name ();
      if ((tmp != nullptr) && utility::strequal (tmp, name))
        return var;
    }
  ...
  while (var != nullptr)
    {
      ...
      if (!copy_xpath_variable (nvar, var))
        return false;
