 ...
 * generated by the CoreDumpGenerator, including Windows-specific mini-dump
...
public:
  ...
  {
    return addMemoryFilter (std::string{
...
{
  if (!DumpTypeUtils::isValid (m_type))
 ...
 */
class LUMEX_UTILITY_API DumpFactory
{
public:
  /**
   ...
   */
  static DumpConfiguration
  createConfiguration (DumpType type, std::error_code &errorCode) noexcept;
...
public:
   ...
   * @throws std::system_error if filesystem operations fail
   */
  static void initialize (DumpConfiguration const &config,
                          bool handleExceptions = true);
   ...
   * remains in a valid state
   */
  static bool generateDump (std::string const &reason = "Manual dump",
                            DumpType dumpType = DumpType::DEFAULT_AUTO);
   ...
   */
  static bool generateDump (std::string const &reason, DumpType dumpType,
   ...
   * @throws std::runtime_error if not initialized
   */
  static bool generateDump (DumpConfiguration const &config,
  ...
  /**
   * @brief Sets the admin group name used by isAdminPrivileges() on UNIX.
   *
   ...
   * @note This method is thread-safe.
   */
  static void setAdminGroupName (std::string const &name) noexcept;
...
private:
  ...
  explicit CoreDumpGenerator (std::string const &dumpDirectory,
...
// Thread safety improvements
   ...
   */
  static bool _createWindowsDump (std::string const &filename,
                                  DumpConfiguration const &config);
#endif
   ...
   */
  static LONG WINAPI
  _windowsExceptionHandler (EXCEPTION_POINTERS *pExInfo) noexcept;
   ...
   */
  static LONG WINAPI _redirectedSetUnhandledExceptionFilter (
...
#endif
  static void _logDumpCreationSuccess (std::string const &filename,
                                       size_t size,
                                       DumpType dumpType) noexcept;

  // Atomic file operations to prevent TOCTOU race conditions
  static bool _createFileAtomically (std::string const &filename,
...
// Security and validation functions
  ...
  static std::string
  _sanitizeFilenameComponent (std::string const &component) noexcept;
  static std::string _sanitizeLogMessage (std::string const &message) noexcept;
  static std::string
  _sanitizeLogMessageForAdmin (std::string const &message) noexcept;
  ...
  static void _logMessage (std::string const &message,
  ...
  static void
  _startPerformanceMonitoring (PerformanceMetrics &metrics) noexcept;
  static void _endPerformanceMonitoring (PerformanceMetrics &metrics,
                                         bool success) noexcept;
  static void
  _logPerformanceMetrics (PerformanceMetrics const &metrics) noexcept;
...
inline void
CoreDumpGenerator::initialize (std::string const &dumpDirectory,
                               DumpType dumpType, bool handleExceptions)
{
  DumpConfiguration config = DumpFactory::createConfiguration (dumpType);
  config.setDirectory (dumpDirectory.empty ()
                           ? _getExecutableDirectory () + "/dumps"
                           : dumpDirectory);
  initialize (config, handleExceptions);
}

inline void
CoreDumpGenerator::initialize (DumpConfiguration const &config,
  ...
  try
    {
      ...
      // Set dump directory
      s_dumpDirectory = config.getDirectory ().empty ()
                            ? _getExecutableDirectory () + "/dumps"
                            : config.getDirectory ();

      // Validate and sanitize directory path
      s_dumpDirectory = _sanitizePath (s_dumpDirectory);
      if (!_validateDirectory (s_dumpDirectory))
      ...
      // Log configuration
      _logMessage ("Dump type: "
                       + DumpFactory::getDescription (config.getType ()),
                   false);
      if (config.getMaxSizeBytes () > 0)
        _logMessage ("Max size: " + std::to_string (config.getMaxSizeBytes ())
                         + " bytes",
                     false);
    ...
    {
      _logMessage ("Failed to initialize CoreDumpGenerator: "
                       + std::string (exc.what ()),
                   true);
      throw;
    }
    ...
    {
      _logMessage ("Failed to initialize CoreDumpGenerator: Unknown error",
                   true);
...
CoreDumpGenerator::generateDump (std::string const &reason, DumpType dumpType)
{
  ...
  try
    ...
    {
      _logMessage ("Failed to generate dump: " + std::string (exc.what ()),
                   true);
      return false;
    }
}
...
CoreDumpGenerator::getCurrentDumpType () noexcept
{
  return s_currentConfig
      .getType (); // s_currentConfig is only modified during
                   // initialization, which is single-threaded
}
...
CoreDumpGenerator::isAdminPrivileges () noexcept
{
  ...
  // setAdminGroupName(). An empty name (the default) and a name that cannot be
  // resolved keep the root-only behaviour.
  ...
  try
    {
      ...
      if (sigaction (signum, &sa, nullptr) == 0)
        {
          _logMessage ("Custom signal handler registered for signal "
...
{
  if (m_acquired)
    _releaseOperationSlot ();
}

// Performance monitoring implementation
inline void
CoreDumpGenerator::_startPerformanceMonitoring (
...
inline void
CoreDumpGenerator::_endPerformanceMonitoring (PerformanceMetrics &metrics,
                                              bool success) noexcept
{
...
inline void
CoreDumpGenerator::_logPerformanceMetrics (
      ...
      if (has_kernel_specifiers && has_our_directory)
        {
          ...
          _logMessage ("  Dumps will be created directly in: "
                           + s_dumpDirectory,
                       false);
        ...
        {
          _logMessage ("WARNING: core_pattern did not change to our pattern "
                       "(sudo likely failed)",
                       false);
          ...
          if (!pattern_before_change.empty ()
              ...
              && pattern_before_change.find ("systemd-coredump")
                     != std::string::npos)
            {
              _logMessage ("INFO: Attempting to restore original "
          ...
          if (!pattern_after_change.empty () && pattern_after_change[0] == '|'
              ...
              try
                {
                  ...
                  _logMessage ("INFO: Using systemd-coredump with instant "
                               "extraction fallback",
                               false);
                  _logMessage ("  Dumps will be extracted from systemd to: "
                                   + s_dumpDirectory,
                               false);
                }
              catch (std::exception const &exc)
                {
                  _logMessage ("Failed to start systemd fallback monitor: "
                                   + std::string (exc.what ()),
                               true);
                }
            }
            ...
            {
              _logMessage ("WARNING: Using system default core_pattern",
                           false);
              _logMessage ("  Current pattern: " + pattern_after_change,
                           false);
              _logMessage ("  Dumps may be created in current directory or "
...
CoreDumpGenerator::_setupWindowsHandlers ()
{
  try
    ...
    {
      _logMessage ("Failed to setup Windows handlers: "
                       + std::string (exc.what ()),
                   true);
      throw;
    }
...
inline LONG WINAPI
CoreDumpGenerator::_windowsExceptionHandler (
  ...
  try
    {
      ...
      if (hFile == INVALID_HANDLE_VALUE)
        {
          ...
          _logMessage ("Failed to create dump file. Error: "
                           + std::to_string (error),
                       true);
      ...
      if (success == FALSE)
        {
          DWORD error = GetLastError ();
          _logMessage ("MiniDumpWriteDump failed with error: "
                           + std::to_string (error),
                       true);
        }
      else
        {
          ...
          if (narrowFilename.empty ())
            {
              _logMessage ("Failed to convert wide string to narrow string",
                           true);
              return EXCEPTION_EXECUTE_HANDLER;
            }
          ...
          _logDumpCreationSuccess (narrowFilename, fileSizeBytes,
...
inline LONG WINAPI
CoreDumpGenerator::_redirectedSetUnhandledExceptionFilter (
    EXCEPTION_POINTERS * /*ExceptionInfo*/) noexcept
{
...
inline bool
CoreDumpGenerator::_createWindowsDump (std::string const &filename,
                                       DumpConfiguration const &config)
{
  try
    {
    ...
    {
      _logMessage ("Exception in _createWindowsDump: "
                       + std::string (exc.what ()),
                   true);
      return false;
    }
...
CoreDumpGenerator::_createManualCoreDump ()
{
  try
      ...
      if (gcore_available == 0)
        {
          ...
          if (result == 0)
            {
              _logMessage ("Manual core dump created using gcore: "
                               + sanitizedFilename,
                           false);
            ...
            {
              _logMessage ("Failed to create core dump using gcore", true);
            }
        }
      else
        {
          ...
          if (gdb_available == 0)
            {
              ...
              if (result == 0)
                {
                  _logMessage ("Manual core dump created using gdb: "
                                   + sanitizedFilename,
                               false);
                ...
                {
                  _logMessage ("Failed to create core dump using gdb", true);
                }
            }
          else
            {
              _logMessage ("Neither gcore nor gdb available for manual core "
                           "dump creation",
                           true);
            }
        }
    ...
    {
      _logMessage ("Failed to create manual core dump: "
                       + std::string (exc.what ()),
                   true);
    }
}
...
CoreDumpGenerator::_restoreCorePattern ()
{
  try
          ...
          if (fd >= 0)
            {
              ...
              if (bytes_written == -1)
                _logMessage ("Failed to restore original core pattern: "
                                 + std::to_string (errno),
                             true);
              else
    ...
    {
      _logMessage ("Failed to restore core pattern: "
                       + std::string (exc.what ()),
                   true);
    }
}
  ...
  try
    {
      ...
      if (stat (systemd_coredump_dir, &st) != 0)
        {
          _logMessage ("systemd-coredump directory not accessible - skipping "
                       "instant monitor",
                       false);
          return;
        }
      ...
      if (watch_fd < 0)
        {
          close (inotify_fd);
          _logMessage ("Failed to add inotify watch on systemd-coredump",
                       true);
          return;
        }

      _logMessage ("INSTANT systemd-coredump monitor ACTIVE (inotify-based)",
                   false);
      _logMessage ("   - Monitoring: " + std::string (systemd_coredump_dir),
                   false);
      _logMessage (
          ...
          if (select_ret > 0 && FD_ISSET (inotify_fd, &readfds))
            {
              ...
              if (len > 0)
                      ...
                      if ((event->mask & (IN_CREATE | IN_MOVED_TO))
                          && event->len > 0)
                        {
                          ...
                          if (filename.find (core_prefix) != std::string::npos)
                            {
                              _logMessage ("NEW CORE DUMP DETECTED: "
                                               + filename,
                                           false);
                              ...
                              _logMessage ("EXTRACTING IMMEDIATELY to: "
                                               + target_file,
                                           false);
                                  ...
                                  if (exit_code == 0)
                                    {
                                      ...
                                      if (stat (target_file.c_str (),
                                        ...
                                        {
                                          _logMessage ("File extracted but "
                                                       "stat() failed",
                                                       true);
                                        }
                                    }
                ...
                {
                  _logMessage ("inotify read error: "
            ...
            {
              _logMessage ("select() error in systemd monitor: "
                               + std::string (strerror (errno)),
                           true);
              break;
            }
    ...
    {
      _logMessage ("Exception in instant systemd monitor: "
                       + std::string (exc.what ()),
                   true);
    }
  catch (...)
  ...
  try
    {
      ...
      if (coredumpctl_available == 0)
                  ...
                  if (!pid_str.empty () && pid_str != "PID")
                    {
                      ...
                      if (result == 0)
                        {
                          _logMessage ("Core dump copied to: "
                                           + filename.str (),
                                       false);
        ...
        {
          _logMessage ("coredumpctl not available for core dump monitoring",
                       true);
        }
    }
  catch (std::exception const &exc)
  ...
  try
    {
      ...
      if (stat (filename.c_str (), &fileStat) == 0)
        {
          _logMessage ("Core dump created successfully. Path: " + filename
        ...
        {
          _logMessage ("Core dump may have been created at: " + filename
          ...
          for (auto const &location : commonLocations)
            {
              ...
              _logMessage ("Check " + location + " for core dump files",
                           false);
            }
        }
    }
  ...
  try
    {
      ...
      _logMessage ("  %t = crash timestamp, %p = PID, %e = executable name",
                   false);
      ...
      if (ret != 0)
        {
          _logMessage ("WARNING: sudo tee failed, attempting direct write",
                       true);
          ...
          if (fd >= 0)
            {
              ...
              if (written > 0)
                {
                  _logMessage ("Direct write successful (no sudo required)",
                               false);
          ...
          _logMessage ("WARNING: Cannot set custom core_pattern - will use "
                       "system default",
                       true);
          _logMessage (
          ...
          _logMessage ("  systemd-coredump fallback monitor will extract "
                       "dumps if active",
                       false);
        }
      else
        {
          _logMessage ("Core pattern set successfully via sudo", false);
        }
    ...
    {
      _logMessage ("Exception in _generateCoreDump: "
                       + std::string (exc.what ()),
                   true);
    }
}
...
inline void
CoreDumpGenerator::_logDumpCreationSuccess (std::string const &filename,
                                            size_t size,
                                            DumpType dumpType) noexcept
{
  try
...
// Atomic file operations to prevent TOCTOU race conditions
inline bool
CoreDumpGenerator::_createFileAtomically (std::string const &filename,
                                          std::string const &content) noexcept
{
  try
    {
      ...
      if (hFile == INVALID_HANDLE_VALUE)
        {
          ...
          if (error != ERROR_FILE_EXISTS)
            _logMessage ("Failed to create file atomically. Error: "
                             + std::to_string (error),
                         true);
          return false;
        }
      ...
      if (bytesWritten == -1)
        _logMessage ("Failed to write to file: " + std::to_string (errno),
                     true);
      close (fd);
...
inline bool
CoreDumpGenerator::_createDirectoryAtomically (
  ...
  try
    {
      ...
      _logMessage ("Failed to create directory atomically. Error: "
                       + std::to_string (error),
                   true);
      return false;
#else
...
CoreDumpGenerator::_getExecutableDirectory () noexcept
{
  try
    ...
    {
      _logMessage ("Exception in _getExecutableDirectory: "
                       + std::string (exc.what ()),
                   true);
      return ".";
    }
...
DumpFactory::createWindowsConfiguration (DumpType type)
{
  ...
  switch (type)
    ...
    default:
      ...
      config.setMaxSizeBytes (256ULL * 1024ULL
...
DumpFactory::createUnixConfiguration (DumpType type)
{
  ...
  switch (type)
    ...
    default:
      // Use default settings with reasonable limits
      config.setMaxSizeBytes (128ULL * 1024ULL
...
inline void
CoreDumpGenerator::_logMessage (std::string const &message,
...
CoreDumpGenerator::_unhandledExceptionHandler ()
{
  try
  ...
  catch (...)
    {
      ...
      if (bytes_written == -1)
        _logMessage ("Failed to write to stderr: " + std::to_string (errno),
                     true);
#else
      // Windows fallback
                               ...
                               CRYPT_VERIFYCONTEXT))
        {
          if (CryptGenRandom (hProv, static_cast<DWORD> (RANDOM_BYTES),
          ...
          else
            {
              ...
              CryptReleaseContext (hProv, 0);
              _logMessage ("CRITICAL: CryptGenRandom failed - falling back to "
                           "insecure method",
                           true);
...
inline std::string
CoreDumpGenerator::_sanitizeFilenameComponent (
...
inline std::string
CoreDumpGenerator::_sanitizeLogMessageForAdmin (
      ...
      if (lastSlash != std::string::npos)
        {
          ...
          if (!_createDirectoryRecursive (parent))
            return false;
        }
