  ...
  try
    {
      ...
      if(!appImageMode.empty() && appImageMode == "1"
      ...
      else
        {
          ...
          Lumex::Path crashesDir = Lumex::Path(homeDir) / Lumex::Path(".local")
                                   / Lumex::Path("share")
                                   / Lumex::Path("LumReportViewer")
                                   / Lumex::Path("crashes");
          Lumex::Filesystem::create_directory(crashesDir);
      ...
      Lumex::Filesystem::create_directory(
      ...
      if(homeDir.empty())
        {
          ...
          dir = Lumex::Filesystem::temp_directory_path().value()
                / Lumex::Path("LumReportViewer") / Lumex::Path("crashes");
        }
      else
        {
          dir = Lumex::Path(homeDir) / Lumex::Path(".local")
                / Lumex::Path("share") / Lumex::Path("LumReportViewer")
                / Lumex::Path("crashes");
        }
    }
#else
  Lumex::Path dir{
    Lumex::Filesystem::LumexFilesystem::get_exe_path().parent_path()
